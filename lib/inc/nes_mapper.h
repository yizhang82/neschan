...
// NROM-383 and NROM-368 and NROM-320
//
class nes_mapper_474 : public nes_mapper
{
public:
    nes_mapper_474(shared_ptr<vector<uint8_t>>& prg_rom, shared_ptr<vector<uint8_t>>& chr_rom, bool vertical_mirroring, uint8_t submapper_number)
        :_prg_rom(prg_rom), _chr_rom(chr_rom), _vertical_mirroring(vertical_mirroring), _submapper_number(submapper_number)
    {
...
public :
...

    {
        ...
        case 474: mapper = make_shared<nes_mapper_474>(prg_rom, chr_rom, vertical_mirroring, submapper_number); break;
        default:
